...
class LUConnectClient:
    ...
    def authenticate(self, username, password, is_register=False):
        ...
        auth_data = {"action": action, "username": username, "password": password}
    ...
    def send_file_init(self, file_path):
        ...
        msg = {"type": "file_transfer_init", "filename": filename}
        self.send_message(msg)
    ...
    def download_file(self, file_id):
        ...
        msg = {"type": "command", "command": "download_file", "file_id": file_id}
        self.send_message(msg)
...
class ClientUI:
    ...
    def setup_ui(self):
        ...
        self.status_label = tk.Label(
            self.main_frame, text="Connecting to server...", fg="blue"
        )
        ...
        self.register_btn = tk.Button(
            btn_frame, text="Register", command=self.register, width=10
        )
        ...
        self.chat_display = scrolledtext.ScrolledText(
            chat_display_frame, state="disabled", height=20
        )
        ...
        self.file_btn = tk.Button(
            self.message_frame, text="Send File", command=self.send_file
        )
    ...
    def send_message(self, event=None):
        ...
        if message:
            ...
            self.chat_display.insert(
                tk.END, f"[{timestamp}] {self.client.username}: {message}\n"
            )
    ...
    def send_file(self):
        ...
        file_path = filedialog.askopenfilename(
            filetypes=[("Documents", "*.docx *.pdf"), ("Images", "*.jpg *.jpeg")]
        )
        if file_path:
    ...
    def display_file_notification(self, message):
        ...
        # file notification
        self.chat_display.insert(
            tk.END, f"[{timestamp}] {sender} shared a file: {filename}\n", "system"
        )
        # download button
    ...
    def save_received_file(self, message):
        ...
        save_path = filedialog.asksaveasfilename(
            initialfile=filename, defaultextension=os.path.splitext(filename)[1]
        )
        if save_path:
            try:
