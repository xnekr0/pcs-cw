...
class LUConnectServer:
    ...
    def start(self):
        ...
        waiting_queue_thread = threading.Thread(target=self.manage_waitqueue, daemon=True)
    ...
    def acceptconn(self, client_socket, address):
        ...
        if self.connection_semaphore.acquire(blocking=False):
            ...
            print(f"Client {address} added to waiting queue. Position: {len(self.waiting_queue)}")
    ...
    def handleclient(self, client_socket, address):
        ...
        try:
            ...
            self.broadcast_message({
                ...
                "timestamp": datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S")
            }, None)  # None means don't exclude any client
            ...
            receive_thread = threading.Thread(
                target=client_handler.receive_chat,
                daemon=True
            )
        ...
        #exeception handling
            ...
            if hasattr(client_handler, 'username') and client_handler.username in self.active_clients:
            ...
            client_socket.close()
            self.connection_semaphore.release()
            print(f"Client {address} disconnected - connection slot released") #DEBUG STATEMENT
            #update active users
            self.broadcast_userstatus()
            #check waiting queue
    ...
    def broadcast_message(self, message, exclude_username=None):
        ...
        for username, client in self.active_clients.items():
            if username != exclude_username:  # Don't send to excluded client (typically the sender)
                try:
    ...
    def manage_waitqueue(self):
        ...
        while True:
            ...
            if self.waiting_queue:
                ...
                self.waiting_queue = [client for client in self.waiting_queue if client["socket"].fileno() != -1]
...
class ClientHandler:
    ...
    def authenticate(self):
        try:
            ...
            if auth_data["action"] == "register":
                #registration
                result = auth_handler.register_user(auth_data["username"], auth_data["password"])
                if result:
                    self.username = auth_data["username"]
                    self.authenticated = True
                    print(f"User {self.username} registered successfully")  # Debug
                    #send auth success message
                    self.send_message({"type": "auth_success", "username": self.username})
                    return True
                else:
                ...
                #login
                result = auth_handler.authenticate_user(auth_data["username"], auth_data["password"])
                if result:
                    self.username = auth_data["username"]
                    self.authenticated = True
        ...
        while True:
            try:
                ...
                while len(message_data) < message_length:
                    chunk = self.client_socket.recv(min(4096, message_length - len(message_data)))
                    if not chunk:
                        print(f"Connection closed during message reception for {self.username}")
                        return
                    message_data += chunk
    ...
    def process_chat_message(self, message):
        ...
        #send to all clients
        self.server.broadcast_message(message, self.username)  # Don't send back to sender
    ...
    def handle_file_transfer_data(self, message):
        ...
        self.server.broadcast_message(file_notification, None)  # Notify everyone including sender
    ...
    def handle_file_download(self, file_id):
        ...
        if file_record:
            ...
            file_msg = {
                ...
                "sender": sender,
                "filename": filename,
                "data": decrypted_data.decode('latin-1'),  # Convert bytes to string for JSON
                "timestamp": datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S")
            }
    ...
    def authenticate_user(self, username, password):
        ...
        try:
            ...
            if user_record:
                ...
                print(f"Stored password hash for {username}: {stored_password}")  # Debug line
                #handle password
